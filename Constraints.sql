...
INSERT INTO key_table (surrogate_column)VALUES (2);
...
-- FOREIGN KEY 제약조건으로 참조되어지고 있는 테이블의 컬럼에 값이 존재하지 않으면 삽입, 수정이 불가능
...
DELETE FROM key_table WHERE priamry_column = 1;
...
CREATE TABLE optional_foreign_table (
    priamry_column INT PRIMARY KEY,
    foreign_column INT,
